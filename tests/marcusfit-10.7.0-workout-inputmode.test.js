...
assert(/class="wo-set-wt" type="text" inputmode="decimal"/.test(blocks[0].innerHTML)&&blocks[0].innerHTML.includes('aria-label="Use text keyboard for this load"')&&blocks[0].innerHTML.includes('>ABC</button>'),"nonnumeric prefilled load did not default to a decimal keypad with an ABC escape hatch");
assert(/class="wo-set-wt" type="text" inputmode="decimal"/.test(blocks[1].innerHTML)&&blocks[1].innerHTML.includes('aria-label="Use text keyboard for this load"')&&blocks[1].innerHTML.includes('>ABC</button>'),"empty load did not default to a decimal keypad with a text-keyboard escape hatch");
...
context.renderWoExercises();assert.strictEqual(blocks.length,2);assert(/value="Bodyweight \+ red band"[^>]*data-field="wt"/.test(blocks[0].innerHTML)&&blocks[0].innerHTML.includes('inputmode="decimal"')&&blocks[0].innerHTML.includes('inputmode="numeric"')&&blocks[1].innerHTML.includes('inputmode="decimal"'),"rerender changed the raw load or lost default keyboard hints");
...
modeInput.value="Bodyweight + red band";assert.strictEqual(modeInput.value,"Bodyweight + red band","ABC mode did not preserve practical nonnumeric editing");
