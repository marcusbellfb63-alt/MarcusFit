...
const sharedHabitPresentation=[/function mfIconMarkup\(name,className\)\{[^\n]+\}/,/function mfHabitDisplayName\(rawName\)\{[\s\S]*?\n\}(?=\nfunction mfHabitIconName)/,/function mfHabitIconName\(id\)\{[\s\S]*?\n\}/].map(pattern=>shared.match(pattern)[0]).join("\n");vm.runInContext(sharedHabitPresentation,habitContext);
