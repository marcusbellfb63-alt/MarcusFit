...
const root=path.resolve(__dirname,".."),source=fs.readFileSync(path.join(root,"assets/js/features/15-stats.js"),"utf8"),shared=fs.readFileSync(path.join(root,"assets/js/features/13-shared-ui.js"),"utf8"),basketball=fs.readFileSync(path.join(root,"assets/js/features/22-basketball.js"),"utf8"),habits=fs.readFileSync(path.join(root,"assets/js/features/20-habits.js"),"utf8"),recurring=fs.readFileSync(path.join(root,"assets/js/features/19-recurring-adherence.js"),"utf8"),html=fs.readFileSync(path.join(root,"index.html"),"utf8");
...
const definitions={
  ...
  "habit-weekly":{id:"habit-weekly",name:"Weekly",icon:"fire",active:true,schedule:{type:"weekly_count",targetCount:2,weekStartsOn:0},createdAt:"2026-06-07T12:00:00.000Z",archivedAt:null},
...
const sharedHabitPresentation=shared.slice(0,shared.indexOf("function mfAdaptCoreSyncOwnedStatusText"));vm.runInContext(sharedHabitPresentation,habitContext);
...
habitContext.p7SetStatsRange("all");const rendered=habitContext.p7CalcAnalytics();habitContext.p7RenderAnalytics();assert.strictEqual(rendered.range.label,"All history");assert.strictEqual(rendered.habits.breakdown.find(item=>item.id==="habit-old").done,1);assert(analyticsContainer.innerHTML.includes("Training Load · All history"));assert(analyticsContainer.innerHTML.includes('#mf-icon-fire'),"Habit Stats did not render the stored canonical SVG icon");assert.strictEqual(habitWrites,0,"calculating or rendering Habit analytics wrote storage");
