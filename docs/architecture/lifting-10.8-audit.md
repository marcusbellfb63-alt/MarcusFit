...
Normal workout rendering first looks for an exact saved exercise record on the selected date. When one exists, the Next session card uses that stored row as its subject and identifies the selected date; when none exists, it ignores unfinished form rows and falls back only to the latest compatible record strictly earlier than the selected date. It never falls forward to a later workout, and its displayed best-performance evidence is bounded to the same subject date. Post-save review, AI Export, and diagnostics use the same explicit saved-subject context containing the exact workout date. The history reader excludes that subject date from prior comparisons, keeps only chronologically preceding comparable records, and adds the completed subject once when counting ceiling evidence. This makes today's completed save eligible immediately across every display/export/debug path, prevents same-date overwrite/edit double-counting, and prevents a backdated record from comparing against itself or a later workout. The context is derived only from saved exercise rows; edits made in the form after the last save remain non-saved evidence until the existing date key is overwritten again.
