...
window.mfArchitectureDebug = function(){
  ...
  try {
    summary.systems = {
      ...
      workoutLogging: { storageKeys:["day-YYYY-MM-DD-wo"], functions:["getTodayWoData","dKey"], mutatesBaseP:false, backupCovered:true, exportSyncRelated:true, purpose:"Per-day Detailed set observations or authoritative Per-Lift Simple summaries for completed workouts." },
  ...
  try {
    summary.dataSchemas = {
      ...
      "day-YYYY-MM-DD-wo": { type:"object", shape:"Workout log: legacy/Detailed {gym,dayIdx,exercises:{id:{sets:[{wt,reps,rir}],note?}}}; or Simple adds liftingDetail:'simple' and uses {sets:[],summary:{version:1,setCount,repsFloor,load,rirFloor},note?}. No synthetic set rows.", missingIsValid:true, corruptJsonHandled:"Yes — getTodayWoData() and history readers use try/catch with {} fallback; malformed Simple summaries remain insufficient evidence without rewriting." },
