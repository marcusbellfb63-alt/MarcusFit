...
function p9510GetAdherenceSummary(itemId,weeks,endDate){
  const item=p9510GetItem(itemId||"zepbound"),out={scheduledOccurrences:0,completedOnTime:0,completedLate:0,skipped:0,unresolvedLate:0,pausedExcluded:0,preferenceExcluded:0,eligibleResolved:0,adherencePercent:null,weeks:weeks||8};
  if(!item||!item.enabled||!p9510ScheduleBase(item))return out;
  p9510OccurrenceDates(item,endDate,weeks).forEach(function(date){if(typeof p950IsTrackingEnabled==="function"&&!p950IsTrackingEnabled("modules.recurringAdherence",date)){out.preferenceExcluded++;return;}if(p9510WasPaused(item,date)){out.pausedExcluded++;return;}const state=p9510GetOccurrenceForDate(item,endDate||p9510DateKey(new Date())),r=p9510ResolveOccurrence(item,date),target=r.replacementDate||date;out.scheduledOccurrences++;if(r.status==="completed"){if(r.actualDate<=target)out.completedOnTime++;else out.completedLate++;}else if(r.status==="skipped")out.skipped++;else if(r.status==="paused")out.pausedExcluded++;else if(p9510DayDiff(target,endDate||p9510DateKey(new Date()))>item.graceDays)out.unresolvedLate++;});
...
function p9510GetAdherenceRange(itemId,range){
  const item=p9510GetItem(itemId||"zepbound"),end=range&&range.end||p9510DateKey(new Date()),start=range&&range.start||item&&p9510ScheduleBase(item)||end,out={scheduledOccurrences:0,completedOnTime:0,completedLate:0,skipped:0,unresolvedLate:0,pausedExcluded:0,preferenceExcluded:0,eligibleResolved:0,adherencePercent:null};if(!item||!item.enabled||!p9510ScheduleBase(item))return out;
  let due=p9510GetNextDueDate(item,p9510AddDays(start,-1)),guard=0;while(due&&due<=end&&guard++<1000){if(typeof p950IsTrackingEnabled==="function"&&!p950IsTrackingEnabled("modules.recurringAdherence",due)){out.preferenceExcluded++;}else if(p9510WasPaused(item,due)){out.pausedExcluded++;}else{const r=p9510ResolveOccurrence(item,due),target=r.replacementDate||due;out.scheduledOccurrences++;if(r.status==="completed"){if(r.actualDate<=target)out.completedOnTime++;else out.completedLate++;}else if(r.status==="skipped")out.skipped++;else if(r.status==="paused")out.pausedExcluded++;else if(p9510DayDiff(target,end)>item.graceDays)out.unresolvedLate++;}due=p9510AddDays(due,7*Math.max(1,item.schedule.interval||1));}
  out.eligibleResolved=out.completedOnTime+out.completedLate+out.skipped+out.unresolvedLate;out.adherencePercent=out.eligibleResolved?Math.round(100*(out.completedOnTime+out.completedLate)/out.eligibleResolved):null;return out;
}
function p9510RenderStats(){
  const el=document.getElementById("p9510StatsSummary");if(!el)return;const item=p9510GetItem("zepbound");if(!item||!item.enabled){el.style.display="none";return;}const range=typeof p7GetStatsRange==="function"?p7GetStatsRange():{start:p9510AddDays(p9510DateKey(new Date()),-55),end:p9510DateKey(new Date()),label:"Recent 8 weeks"},s=p9510GetAdherenceRange("zepbound",range);el.style.display="block";el.innerHTML="<strong>Zepbound · "+range.label+"</strong><br>Scheduled "+s.scheduledOccurrences+" · On time "+s.completedOnTime+" · Late "+s.completedLate+" · Skipped "+s.skipped+" · Unresolved late "+s.unresolvedLate+"<br>Adherence "+(s.adherencePercent==null?"—":s.adherencePercent+"%")+" (completed ÷ completed, skipped, and unresolved late occurrences; "+s.preferenceExcluded+" preference-off and paused/upcoming excluded)";
}
function p9510BuildAdherenceExport(){
  ...
  return "--- SCHEDULED ADHERENCE ---\nMaster tracking preference: "+(typeof p950IsTrackingEnabled!=="function"||p950IsTrackingEnabled("modules.recurringAdherence")?"on":"intentionally off")+"\nZepbound tracking: "+(item.enabled?(item.paused?"paused":"enabled"):"disabled")+"\nSchedule: "+(p9510ScheduleBase(item)?["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][weekday]+" weekly; grace "+item.graceDays+" day(s)":"unconfigured")+"\nLast occurrence: "+(prev||"none")+" · "+(resolved.status||"unresolved")+" · source "+resolved.source+"\nNext due: "+(next||"unconfigured")+"\nRecent 8 weeks: scheduled "+s.scheduledOccurrences+", on-time "+s.completedOnTime+", late "+s.completedLate+", skipped "+s.skipped+", unresolved late "+s.unresolvedLate+", preference-off excluded "+s.preferenceExcluded+", adherence "+(s.adherencePercent==null?"n/a":s.adherencePercent+"%")+".\nGuidance: Blank non-due or preference-off days are not misses. Only enabled scheduled occurrences belong in adherence calculations. Completed late, skipped, rescheduled, paused, and unresolved are distinct. Discuss patterns neutrally; do not provide medication dosing/timing instructions or automatically change this schedule.\n\n";
}
