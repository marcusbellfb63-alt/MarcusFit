...
function renderWoExercises(){
  ...
  day.exercises.forEach(ex=>{
    ...
    if(liftingDetail==="simple"){
      ...
      setRowsHTML=`<div class="wo-simple-summary" role="group" aria-label="Per-Lift summary for ${mfWorkoutEscapeAttr(nm)}"><div class="wo-simple-labels"><span>Sets</span><span>Reps</span><span>Weight</span><span>RIR</span></div><div class="wo-simple-fields"><input class="wo-summary-sets" type="text" inputmode="numeric" aria-label="Completed work sets" placeholder="${st}" value="${mfWorkoutEscapeAttr(summary.setCount||"")}" data-exid="${ex.id}" data-field="summarySetCount"><input class="wo-summary-reps" type="text" inputmode="${valueInputMode}" aria-label="Lowest reps completed on any counted work set" placeholder="lowest" value="${mfWorkoutEscapeAttr(summary.repsFloor||"")}" data-exid="${ex.id}" data-field="summaryReps"><div class="wo-set-load-entry"><input class="wo-summary-load wo-set-wt" type="text" inputmode="decimal" aria-label="Common working resistance" placeholder="${mfWorkoutEscapeAttr(ld)}" value="${mfWorkoutEscapeAttr(summary.load||"")}" data-exid="${ex.id}" data-field="summaryLoad"><button type="button" class="wo-load-keyboard-toggle" aria-label="Use text keyboard for this load" aria-pressed="false">ABC</button></div><select class="wo-summary-rir" aria-label="Hardest lowest RIR" data-exid="${ex.id}" data-field="summaryRir">${rirOptsS}</select></div><div class="wo-simple-help">Reps = lowest-rep work set · RIR = hardest (lowest) work set.</div></div>`;
    }else{
...
function p85ExecuteSave(){
  ...
  const fields=[
    ["weight",!tracking||tracking.dailyMetrics.weight,document.getElementById("weightIn").value],
    ["sleep",!tracking||tracking.dailyMetrics.sleep,document.getElementById("sleepIn").value],
    ["protein",!tracking||tracking.dailyMetrics.protein,document.getElementById("proteinIn").value],
    ["water",!tracking||tracking.dailyMetrics.water,document.getElementById("waterIn").value],
    ["bm",!tracking||tracking.dailyMetrics.bowelMovement,toggleStates.bm],
    ["bmNotes",!tracking||tracking.dailyMetrics.bowelMovement,document.getElementById("bmNotes").value],
    ["mood",!tracking||tracking.dailyMetrics.energy,document.getElementById("moodSlider").value],
    ["hunger",!tracking||tracking.dailyMetrics.hunger,document.getElementById("hungerSlider").value],
    ["zep",!tracking||tracking.modules.recurringAdherence,toggleStates.zep],
    ["notes",!tracking||tracking.modules.dailyNotes,document.getElementById("dayNotes").value],
    ["habits",!tracking||tracking.modules.habits,habitState]
  ];
  fields.forEach(function(field){if(field[1])data[field[0]]=field[2];else if(Object.prototype.hasOwnProperty.call(before,field[0]))data[field[0]]=before[field[0]];});
